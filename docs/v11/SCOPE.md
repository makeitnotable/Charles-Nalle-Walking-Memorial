...
| V11-02 | "On desktop and tablet (only if space allows), in the 'Where to Next' section across all chapter pages, left-align the chapter label and address under the map so they align with the left edge of the map and with each other. Then right-align the 'Get directions' button with the right edge of the map, vertically centered with the chapter label and address." | — |
...
| V11-09 | "On desktop and anywhere else this occurs, I'd like the map on the Map page pushed higher so the ferry pin sits farther from the 'TAKE THE WALK' button. All pins should remain visible on every screen size." | — |
