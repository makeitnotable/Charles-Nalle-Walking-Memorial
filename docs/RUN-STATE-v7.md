...
## CURRENT PHASE
**RUN COMPLETE — 2026-08-16 (Fable 5).** Phases 0–8 done; the Part E gate is
met: juror passes 10 (f1dbbb3) and 11 (d12373d) are two consecutive clean
passes by fresh jurors (only P2/P3 fixes between them, per the protocol); live =
HEAD verified after every push; `docs/v7/REVIEW-GUIDE.md` written for Wil;
memory updated. Anything after this line is maintenance.
...
## NEXT ACTION
**None — run complete.** **Juror pass 11 = PASS** (`docs/v7/juror-pass11.md`,
build d12373d; Sheet A phone 9/8.5/9/9, tablet 9/9/9/9, desktop 9/9/9.5/9; ZERO
P0/P1; Sheet B all Met/N-A; both juror-10 P2s confirmed fixed). With pass 10
(PASS, P2/P3 fixes only in between) the gate is met. Landed after pass 11 as
P2/P3 (instrument- and probe-verified, no juror): the ⌘-wheel notice moves into
the doors' row from 1280 up (clear of every pill/door/(i) at 1280/1440/1920 after
a 330 px scroll; 1024–1279 keeps the centred slot), and `apple-touch-icon.png`
drops its alpha channel (`build-favicon.mjs`). If Wil asks for more: the human
queue and residuals are in `docs/v7/REVIEW-GUIDE.md` §4–§5; a new round starts
with a new AUDIT, not with fixes.

(Earlier) **Juror pass 10 = PASS** (`docs/v7/juror-pass10.md`, build f1dbbb3; Sheet A phone
...
| **P8 juror loop (Part E gate)** — 11 fresh-juror passes on the live build (`docs/v7/juror-pass1.md`…`11.md`, protocol `docs/v7/JUROR-PROMPT.md`, scripts `scripts/juror*-*.mjs`): 1 FAIL 2×P1 (production aspects, sheet dead to touch) → 2 FAIL P1 (glued index titles) → 3 FAIL Sheet B → 4 FAIL P1 (chip under Skip) → 5 FAIL P1 (chip wrapped) → **6 PASS** → 7 FAIL P1 (inspect view opened cropped from the page top) → 8 FAIL Sheet B (plaque runt, walk-state race) → 9 FAIL P1 (my `nowrap` regression on People roles) → **10 PASS** → **11 PASS** = the gate. Fixes along the way: `approach()` scrolls the sticky stage flush; desktop map wheel cooperative (⌘/Ctrl to zoom, house chip); walk-card neighbours scale about the near edge (peeks 17/19 px); plaque = authored two-line lockup; strip settle no-ops when unfocused; `.sep-list` separator-aware wrap for ` · ` eyebrows/roles; hint chip + wheel notice in the doors' row ≥ 1280; ☰ kept in inspect mode; scripted focus only after keyboard input; 13rem card floor at 200 % zoom; card button full-width in narrow cards; lens caption two lines on phones; player title two lines at 390; apple-touch-icon opaque. Regressions after each fix: a11y 0/0/0 · states 0/135 · walk 8/8 · museum 5/5 · rag 0/0/0. | 50f9fcb 3396185 eae5219 9fd4401 df0ee6c c586a93 c202f20 29e69f4 480f715 f1dbbb3 d12373d + final | `docs/v7/juror-pass*.md`, `docs/v7/qa/j7fix j8fix j9fix` |
