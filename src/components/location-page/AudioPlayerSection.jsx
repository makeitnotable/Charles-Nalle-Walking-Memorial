...
export default function AudioPlayerSection({ data, id = 'audioplayersection' }) {
    ...
    const mainPlayButtonRef = useRef(null);
    ...
    // Track scroll position to toggle between main and mini player
    useEffect(() => {
        const handleScroll = () => {
            const mainPlayButton = mainPlayButtonRef.current;
            if (!mainPlayButton) return;

            const rect = mainPlayButton.getBoundingClientRect();
            // When top of button goes past top of viewport (rect.top < 0), main player is out of view
            const isVisible = rect.top >= 0;
            setMainPlayerVisible(isVisible);
        };
    ...
    return (
        <>
            ...
            <div 
                id={id} 
                className={clsx('rounded-3xl border-2 border-primary-6 transition-colors duration-300 md:w-[29.296rem] md:h-[26.05rem] lg:w-[32.5rem] lg:h-[29.291rem]', {
                    'bg-primary-4': isPlaying,
                    'bg-primary-3': !isPlaying,
                })}
                style={{
                    opacity: mainPlayerVisible ? 1 : 0,
            ...
            {showMiniPlayer && (
                <div 
                    className='fixed bottom-0 left-0 right-0 w-full z-[999]'
                    style={{
                        opacity: mainPlayerVisible ? 0 : 1,
                        pointerEvents: mainPlayerVisible ? 'none' : 'auto',
                        transition: 'opacity 300ms ease-in-out',
                    }}
                >
