#!/usr/bin/env node
/**
 * Round 24 — the /paintings runway, measured where this container can
 * measure it (docs/rounds/2026-09-21-round-24-plan.md).
 *
 * Why this exists: no instrument here can see Safari's bars, so the runway
 * strips are proved on the two things Chromium CAN show — that the framing
 * the reader sees does not move by a pixel when the bleed is on, and that the
 * strips carry the corridor's own rows, placed where the bars would be.
 *
 * B is stood in the way RUN-STATE records for safe-area insets and
 * map-framing.mjs for the map's runway: an init-script <style> pins
 * `--museum-b` on `html[data-glass] .museum-stage` with !important (the
 * @supports (-webkit-touch-callout) gate never matches in Chromium, so the
 * stage would otherwise carry 0). `?glass=1` sets the attribute. Without the
 * stand-in, `?glass=1` must change nothing — the desktop guarantee.
 *
 * Per phone viewport (390×645 = the iOS layout viewport with the bars
 * expanded; 430×720 as a second size), three sessions — OFF (no flag), ON
 * (`?glass=1`, B stood in), TRACK (`?glass=1&runway=track`) — and per session
 * three positions: rest (scroll 0), stuck (40% of the slot), past (the stills
 * below the hall). Asserted:
 *   · OFF: no strip in the DOM, canvas == stage, runway.b == 0.
 *   · ON, every position: stage box identical to OFF; canvas = stage + 2B,
 *     starting B above it; `--cnwm-chip-y` identical to OFF; the projected
 *     rect of the painting under the walk identical to OFF (±0.5px) — the
 *     framing invariant.
 *   · ON at rest: the bottom strip visible at svh − 48 (48 + B + 48 tall),
 *     the top strip hidden; stuck: both visible, the top one from
 *     −(48 + B) to +48; past: both hidden.
 ...
 *   · TRACK: the path reports "track", the self-check never demotes it, and
 *     the stuck position lands to ±1px.
  ...
  if (standIn) {
    await ctx.addInitScript((b) => {
      ...
      const add = () => {
        ...
        st.textContent = `html[data-glass] .museum-stage{--museum-b:${b}px !important}`;
...
for (const vp of VPS) {
  ...
  /* ── ON ── */
  const on = await open(vp, "?glass=1", true);
  ...
  const bl = await open(vp, "?glass=1&runway=off", true);
  ...
  const trRest = await tr.page.evaluate(SNAPSHOT);
  check(V, "track", "rest", "hall mounted", !trRest.missing, trRest.missing ? "no __museum" : "");
  if (!trRest.missing) {
    check(V, "track", "rest", "path = track", trRest.runway && trRest.runway.path === "track", JSON.stringify(trRest.runway && { path: trRest.runway.path, failed: trRest.runway.trackFailed }));
    stripAsserts(V, "track", "rest", trRest, false, true);
    await tr.page.evaluate(SCROLL_TO_RAIL, 0.4);
    await tr.page.waitForTimeout(1800);
    const trStuck = await tr.page.evaluate(SNAPSHOT);
    check(V, "track", "stuck", "still on the compositor path", trStuck.runway && trStuck.runway.path === "track" && !trStuck.runway.trackFailed, JSON.stringify(trStuck.runway && { path: trStuck.runway.path, failed: trStuck.runway.trackFailed }));
    stripAsserts(V, "track", "stuck", trStuck, true, true);
    framingAsserts(V, "track", "stuck", offStuck, trStuck);
    check(V, "track", "walk", "no page errors", tr.errs.length === 0, tr.errs.join(" | "));
  }
  await tr.ctx.close();
}

/* ── the desktop guarantee: the flag without bars changes nothing ── */
{
  const vp = { w: 1440, h: 900 };
  const V = "1440x900";
  const d = await open(vp, "?glass=1", false);
  const snap = await d.page.evaluate(SNAPSHOT);
  check(V, "flag, no bars", "rest", "hall mounted", !snap.missing, snap.missing ? "no __museum" : "");
  if (!snap.missing) {
    check(V, "flag, no bars", "rest", "no strip in the DOM", snap.strips.length === 0, `${snap.strips.length} strip(s)`);
    check(V, "flag, no bars", "rest", "canvas = stage", near(snap.canvas.height, snap.stage.height, 0.5) && near(snap.canvas.top, snap.stage.top, 0.5), `${r1(snap.canvas.height)} vs ${r1(snap.stage.height)}`);
    check(V, "flag, no bars", "rest", "runway.b = 0", snap.runway && snap.runway.b === 0, JSON.stringify(snap.runway));
    check(V, "flag, no bars", "rest", "no page errors", d.errs.length === 0, d.errs.join(" | "));
  }
  await d.ctx.close();
}
await browser.close();
...
/* ── report ── */
const lines = ["# /paintings runway — measured", "", `base ${BASE} · B ${B} · ${new Date().toISOString()}`, "", "| viewport | session | position | check | ok | detail |", "|---|---|---|---|---|---|"];
