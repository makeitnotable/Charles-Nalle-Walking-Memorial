...
/* v7 I4: the v4 build read Martel Sans + Poppins, both long gone from
   package.json (ENOENT). The lockup is Caslon now — the same two faces the
   site and the curtain use — and carries the CN mark, quiet, bottom-left. */
const font = (p) => readFileSync(`node_modules/@fontsource/${p}`).toString("base64");
const display = font("libre-caslon-display/files/libre-caslon-display-latin-400-normal.woff2");
const text = font("libre-caslon-text/files/libre-caslon-text-latin-400-normal.woff2");
const bg = readFileSync("public/media/site/home-bg-800.avif").toString("base64");
const mark = readFileSync("public/favicon.svg").toString("base64");

const html = `<!doctype html><html><head><meta charset="utf-8"><style>
@font-face{font-family:LCD;src:url(data:font/woff2;base64,${display}) format('woff2');font-weight:400}
@font-face{font-family:LCT;src:url(data:font/woff2;base64,${text}) format('woff2');font-weight:400}
...
.wm{font-family:LCD;font-weight:400;font-size:148px;line-height:.86;letter-spacing:-.03em;
...
.rule{display:flex;align-items:center;gap:14px;font-family:LCT;font-weight:400;font-size:16px;
  letter-spacing:.12em;text-transform:uppercase;color:#ff9770}
.rule i{display:block;width:44px;height:1px;background:#e45b27}
.meta{font-family:LCT;font-weight:400;font-size:16px;letter-spacing:.12em;text-transform:uppercase;
  color:#d0cbc6;margin-top:6px}
.mark{position:absolute;left:44px;bottom:40px;width:64px;height:64px}
</style></head><body>
<div class="bg"></div>
<div class="wrap">
